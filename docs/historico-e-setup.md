...
- **Tema Sistema / Claro / Escuro** — selector no cabeçalho (`src/components/ThemeToggle.tsx`).
  Preferência guardada em `localStorage` (`cucos-theme`); "Sistema" segue o SO.
...
- **Nomes das equipas em pt-PT** — `src/lib/team-names.ts` traduz os nomes da
  API-Football (inglês) para Português de Portugal (ex.: Spain → Espanha).
- **Visual** — fundo com gradientes, cartões com sombra/animação, indicador
  "ao vivo" a pulsar, badges de canais em destaque.
- **Rodapé** — "© 2026 Cuco Enterprise" centrado no fundo de todas as páginas.
