...
export type RadialTeamBadge = {
  name: string;
  id: number;
};

/** Vencedor do jogo do nó filho (oitavos, etc.). */
export function teamBadgeFromFeederSlot(
  slot: BracketSlotData
): RadialTeamBadge | null {
  const winner = winnerFromFeederSlot(slot);
  if (winner) return winner;

  const match = slot.match;
  if (!match || isSyntheticFixture(match.home_team_id)) return null;
  if (match.status === "finished") return null;
...
function winnerFromFeederSlot(slot: BracketSlotData): RadialTeamBadge | null {
  const match = slot.match;
...
/**
 * Ordem vertical das bandeiras (topo→base) alinhada com os feeders no anel:
 * o filho com maior y (mais abaixo no ecrã / mais interior) fica em cima no par.
 */
export function orderTeamsByFeederGeometry(
  matchNodeId: string,
  slot: BracketSlotData,
  nodes: Map<string, RadialLayoutNode>
): [RadialTeamBadge, RadialTeamBadge] | null {
  const node = nodes.get(matchNodeId);
  if (!node || node.isSlot || node.children.length !== 2) return null;

  const feedersTopToBottom = [...node.children].sort(
    (a, b) => (nodes.get(b)?.y ?? 0) - (nodes.get(a)?.y ?? 0)
  );

  const feederTeams = feedersTopToBottom.map((childId) => {
    const child = nodes.get(childId);
    if (!child) return null;
    return teamBadgeFromFeederSlot(child.slot);
  });

  if (feederTeams[0] && feederTeams[1]) {
    return [feederTeams[0], feederTeams[1]];
  }
  ...
  const homeTeam: RadialTeamBadge = {
    name: match.home_team_name,
    id: match.home_team_id,
  };
  const awayTeam: RadialTeamBadge = {
    name: match.away_team_name,
    id: match.away_team_id,
  };

  const topFeederTeam = feederTeams[0];
  const bottomFeederTeam = feederTeams[1];

  if (topFeederTeam && !bottomFeederTeam) {
    const other =
      topFeederTeam.id === homeTeam.id
        ? awayTeam
        : topFeederTeam.id === awayTeam.id
          ? homeTeam
          : null;
    return other ? [topFeederTeam, other] : null;
  }

  if (!topFeederTeam && bottomFeederTeam) {
    const other =
      bottomFeederTeam.id === homeTeam.id
        ? awayTeam
        : bottomFeederTeam.id === awayTeam.id
          ? homeTeam
          : null;
    return other ? [other, bottomFeederTeam] : null;
  }

  if (topFeederTeam && bottomFeederTeam) {
    return [topFeederTeam, bottomFeederTeam];
  }

  const topFeederId = feedersTopToBottom[0]!;
  const topFromChild = teamIdFromFeederChild(topFeederId, nodes, match);
  if (topFromChild == null) return null;

  if (topFromChild === homeTeam.id) return [homeTeam, awayTeam];
  if (topFromChild === awayTeam.id) return [awayTeam, homeTeam];

  return null;
}

function teamIdFromFeederChild(
  childId: string,
  nodes: Map<string, RadialLayoutNode>,
  qfMatch: Match
): number | null {
  const childSlot = nodes.get(childId)?.slot;
  if (!childSlot) return null;

  const badge = teamBadgeFromFeederSlot(childSlot);
  if (badge) return badge.id;

  const childMatch = childSlot.match;
  if (!childMatch) return null;

  const inChild = (id: number) =>
    id === childMatch.home_team_id || id === childMatch.away_team_id;

  if (inChild(qfMatch.home_team_id)) return qfMatch.home_team_id;
  if (inChild(qfMatch.away_team_id)) return qfMatch.away_team_id;
